...
RoutePlanner::RoutePlanner(RouteModel &model, float start_x, float start_y, float end_x, float end_y): m_Model(model) {
    ...
    std::cout << "Start node coords: (" << start_x << "," << start_y << ")\n";
    std::cout << "Closest to start node coords: (" << start_node->x << "," << start_node->y << ")\n";
}
...
void RoutePlanner::AddNeighbors(RouteModel::Node *current_node) {
    std::cout << "Adding Neighbours to node(" << current_node->x << "," << current_node->y << ")\n";
 ...
 */
void NodeSort(std::vector<RouteModel::Node*> list) {
  std::sort(list.begin(), list.end(), Compare);
}
RouteModel::Node *RoutePlanner::NextNode() {
    //std::vector<RouteModel::Node*> open_list
    NodeSort(open_list);
    RouteModel::Node* LowestNode = *open_list.end();
    open_list.pop_back();
    return LowestNode;
}
...
std::vector<RouteModel::Node> RoutePlanner::ConstructFinalPath(RouteModel::Node *current_node) {
   ...
   path_found.insert(path_found.begin(), *PathNode);
   while (PathNode->parent != start_node)
    {
        PathNode = current_node->parent;
